...
import IssueTable from '../archive/archive';
        ...
        <div className='blog-page'>
            <header className='height-75'>
                <div className='container h-100 d-flex flex-column align-items-center justify-content-center text-light'>
                    <h1 className='text-center fw-semibold'>Archive</h1>
                    <p className='text-center w-75 mb-5'>
                     W.e.f. Vol. 9-Issue 1 i.e. January 2017 BJIT publication can be accessed free of cost, if one has the Member ID and password at bvicam.ac.in. For details, click here.</p>
                </div>
            </header>
            ...
            <div className='bg-body-tertiary py-5'>
                <div className="container">
                    <div className="row g-4">
                        {blogs.map((blog) => (
                            <div key={blog.id} className='col-md-6 col-lg-4'>
                                <Link to="/archive" className='text-decoration-none' onClick={() => handleCardClick(blog)}>
                                    <Card className='h-100 shadow scale-hover-effect bg-dark text-light border-0'>
                                        <Card.Img variant="top" src={blog.img} />
                                        <Card.Body className='p-md-5 d-flex flex-column align-items-center'>
                                            <Card.Title className='fs-2 mb-4'>{blog.title}</Card.Title>
            ...
            {showIssueTable && <IssueTable selectedBlog={selectedBlog} />}
        </div>
    );
}
